...
void LLWebRTCVoiceClient::init(LLPumpIO *pump)
{
    ...
    LLEventPumps::instance().obtain("mainloop").listen("LLWebRTCVoiceClient", boost::bind(&LLWebRTCVoiceClient::onMainLoop, this, _1));
    refreshDeviceLists();
}

// voiceConnectionCoro() runs on a background boost::thread (see its launch
// site) but its work touches main-thread-only viewer state. It posts that
// work here; onMainLoop() (called every frame via the "mainloop" LLEventPump)
// drains and runs it on the main thread.
void LLWebRTCVoiceClient::postToMainThread(std::function<void()> fn)
{
    std::lock_guard<std::mutex> lock(mMainThreadWorkMutex);
    mMainThreadWork.push_back(std::move(fn));
}

bool LLWebRTCVoiceClient::onMainLoop(const LLSD &data)
{
    std::deque<std::function<void()>> pending;
    {
        std::lock_guard<std::mutex> lock(mMainThreadWorkMutex);
        pending.swap(mMainThreadWork);
    }
    for (auto &fn : pending)
    {
        fn();
...
{
    // TEMP DIAGNOSTIC - revert once resolved
    LL_INFOS("Voice") << "DIAGDEV tuningStart mIsInTuningMode=" << mIsInTuningMode << LL_ENDL;
    if (!mIsInTuningMode)
    {
...
{
    // TEMP DIAGNOSTIC - revert once resolved
    LL_INFOS("Voice") << "DIAGDEV setCaptureDevice name='" << name << "'" << LL_ENDL;
...
void LLWebRTCVoiceClient::voiceConnectionCoro()
{
    ...
    try
    {
        ...
        while (!sShuttingDown)
        {
            ...
            // Everything below touches main-thread-only viewer state (gAgent,
            // LLViewerParcelMgr, the voice session/connection state machine, etc.),
            // so it must not run directly on this background thread. Post it to
            // the main thread via postToMainThread()/onMainLoop() instead of
            // executing it here.
...
void LLVoiceWebRTCConnection::OnPeerConnectionClosed()
{
    ...
    else if (LLWebRTCVoiceClient::isShuttingDown())
    {
        ...
        // Called directly by the underlying webrtc peer connection on one of its own threads,
        // not the viewer main thread - LL_INFOS() touches main-thread-only viewer systems
        // (LLFrameTimer), so deliberately not logging here. See OnAudioEstablished() above.
        setVoiceConnectionState(VOICE_STATE_CLOSED);
    }
}
void LLVoiceWebRTCConnection::OnAudioEstablished(llwebrtc::LLWebRTCAudioInterface *audio_interface)
{
    ...
    // Called directly by the underlying webrtc peer connection on one of
    // its own threads, not the viewer main thread - defer the actual
    // state transition (setVoiceConnectionState logs via LL_INFOS(),
    // which touches main-thread-only viewer systems) to the main thread
    // via connectionStateMachine(). Deliberately no LL_DEBUGS() here for
    // the same reason.
    mWebRTCAudioInterface = audio_interface;
    mAudioEstablished = true;
}
...
// llwebrtc callback
// Called directly by the underlying webrtc data channel on one of its own
// threads, not the viewer main thread. OnDataReceivedImpl() touches
// main-thread-only viewer systems (participant lists, LLAvatarNameCache,
// UI observers), so just queue the raw message here and let
// processQueuedDataChannelMessages() (called from connectionStateMachine()
// on the main thread) do the actual work.
void LLVoiceWebRTCConnection::OnDataReceived(const std::string& data, bool binary)
{
    std::lock_guard<std::mutex> lock(mDataChannelQueueMutex);
    mDataChannelQueue.emplace_back(data, binary);
}

void LLVoiceWebRTCConnection::processQueuedDataChannelMessages()
{
    std::deque<std::pair<std::string, bool>> pending;
    {
        std::lock_guard<std::mutex> lock(mDataChannelQueueMutex);
        pending.swap(mDataChannelQueue);
    }
    for (auto &msg : pending)
    {
        OnDataReceivedImpl(msg.first, msg.second);
    }
}
...
{
    processIceUpdates();
    processQueuedDataChannelMessages();
    ...
    switch (getVoiceConnectionState())
    {
        ...
        case VOICE_STATE_CONNECTION_WAIT:
            ...
            else if (mAudioEstablished)
            {
                // OnAudioEstablished() (webrtc thread) already set
                // mWebRTCAudioInterface - safe to make the real state
                // transition here, on the main thread.
