...
private:
    ...
    // voiceConnectionCoro() runs its per-iteration work on a dedicated
    // boost::thread (not a true main-thread-cooperative LLCoros coroutine -
    // see the comment at its launch site), but that work touches main-thread-only
    // viewer state (gAgent, LLViewerParcelMgr, the voice state machine, etc.).
    // Rather than patch each unsafe call individually, the thread posts its
    // per-iteration work here and onMainLoop() (listening on the existing
    // "mainloop" LLEventPump) drains and runs it on the main thread.
    bool onMainLoop(const LLSD &data);
    void postToMainThread(std::function<void()> fn);
    std::mutex mMainThreadWorkMutex;
    std::deque<std::function<void()>> mMainThreadWork;
    ...
    public std::enable_shared_from_this<LLVoiceWebRTCConnection>
{
  ...
  protected:
    ...
    // OnAudioEstablished() is invoked directly by the underlying webrtc
    // peer connection on one of ITS OWN threads, not the viewer main
    // thread. mWebRTCAudioInterface can be set from there (a plain
    // pointer write), but the actual state-machine transition (which
    // logs via LL_INFOS(), touching main-thread-only viewer systems like
    // LLFrameTimer) must happen on the main thread. This flag is that
    // handoff: set here from the webrtc thread, polled and consumed by
    // connectionStateMachine() on the main thread.
    std::atomic<bool> mAudioEstablished { false };

    // OnDataReceived() (webrtc data-channel callback) is likewise invoked
    // directly on a webrtc-internal thread. Its real work (OnDataReceivedImpl)
    // touches participant/name-cache/UI systems that are main-thread-only
    // (e.g. LLAvatarNameCache, which itself uses LLFrameTimer), so instead of
    // processing inline, queue the raw message here and drain it from
    // connectionStateMachine() on the main thread.
    void processQueuedDataChannelMessages();
    std::mutex mDataChannelQueueMutex;
    std::deque<std::pair<std::string, bool>> mDataChannelQueue;
};
