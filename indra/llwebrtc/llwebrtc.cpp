...
#include <algorithm>
#include <string.h>
#include <cstdio> // TEMP DIAGNOSTIC - revert once resolved
...
// Definition for the extern declaration in llwebrtc.h (see comment there for
// why this isn't an inline variable).
std::atomic<bool> gWebRTCUpdateDevices = false;
...
void LLWebRTCImpl::workerDeployDevices()
{
    ...
    {
        setRecRes = mDeviceModule->SetRecordingDevice((webrtc::AudioDeviceModule::WindowsDeviceType)recordingDevice);
    }
    else
    {
        setRecRes = mDeviceModule->SetRecordingDevice(recordingDevice);
    }
#else
    setRecRes = mDeviceModule->SetRecordingDevice(recordingDevice);
#endif
    int32_t initMicRes = mDeviceModule->InitMicrophone();
    mDeviceModule->SetStereoRecording(false);
    int32_t initRecRes = mDeviceModule->InitRecording();
    // TEMP DIAGNOSTIC - revert once resolved
    {
        FILE* f = fopen("webrtc_diag.txt", "a");
        if (f)
        {
            fprintf(f, "workerDeployDevices: mRecordingDevice='%s' resolvedIndex=%d listSize=%zu "
                       "SetRecordingDevice=%d InitMicrophone=%d InitRecording=%d\n",
                    mRecordingDevice.c_str(), (int)recordingDevice, mRecordingDeviceList.size(),
                    (int)setRecRes, (int)initMicRes, (int)initRecRes);
            fclose(f);
        }
    }
