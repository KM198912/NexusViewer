...
public:
    ...
    int32_t RegisterAudioCallback(webrtc::AudioTransport* engine_transport) override
    {
        ...
        int32_t res = inner_->RegisterAudioCallback(&audio_transport_);
        // TEMP DIAGNOSTIC - revert once resolved
        {
            FILE* f = fopen("webrtc_diag.txt", "a");
            if (f)
            {
                fprintf(f, "RegisterAudioCallback: engine_transport=%p result=%d\n", (void*)engine_transport, (int)res);
                fclose(f);
            }
        }
        return res;
...
// Only supported on iOS.
    ...
    void  SetTuning(bool tuning, bool mute)
    {
        ...
        {
            // TEMP DIAGNOSTIC - revert once resolved
            int32_t initRes = inner_->InitRecording();
            int32_t startRes = inner_->StartRecording();
            bool recordingNow = inner_->Recording();
            {
                FILE* f = fopen("webrtc_diag.txt", "a");
                if (f)
                {
                    fprintf(f, "SetTuning(true): InitRecording=%d StartRecording=%d Recording()=%d\n",
                            (int)initRes, (int)startRes, (int)recordingNow);
                    fclose(f);
                }
            }
            inner_->StopPlayout();
